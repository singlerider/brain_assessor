...
class ColorMeaningGame(boolgame.BoolGame, Colors):
    ...
    def __init__(self, width=700, height=500):
        ...
        self.colors = [self.red[:], self.white[
            :], self.green[:], self.yellow[:]]
        self.colornames = ['red', 'white', 'green', 'yellow']

        boolgame.BoolGame.__init__(
            self, self.title, width, height, description=self.descriptiontext, color=self.color)
        self.loadGameSettings()

        self.leftBtn = RectangularButton('yellow', 1, self.width // 2 - 120, self.description.y -
                                         100, w=120, h=100, color=self.solarized, textcolor=self.red, textsize=22)
        self.rightBtn = RectangularButton('white', 1, self.width // 2 + 20, self.description.y -
                                          100, w=120, h=100, color=self.solarized, textcolor=self.yellow, textsize=22)
    ...
    def addNew(self):
        ...
        self.answer = maths.weightedRandomIndex(
            [0.5, 0.5])  # 0.5 for 0 i.e wrong answer
        leftid = maths.randint(0, 3)
        leftnameid = maths.randint(0, 3)

        if self.answer == 0:
            # 0.4 chance left color = right color
            rightid_pdf = popList(leftid, 0.2) + \
                [0.4] + popList(3 - leftid, 0.2)
            rightid = leftnameid
            ...
            rightid = leftnameid
            rightnameid_pdf = popList(leftid, 0.2) + \
                [0.4] + popList(3 - leftid, 0.2)
