...
class ColorMatchGame(boolgame.BoolGame, colors.Colors):
    ...
    def __init__(self, width=700, height=500):
        ...
        self.curshape = draw.circle(
            self.width // 2 - 75, 300, 75, filled=True, color=self.colors[0])
        self.curshape.draw()
        self.previous = 0
    ...
    def addNew(self):
        ...
        if cshape == 'square':
            self.curshape = draw.square(
                self.width // 2 - 75, 300, 150, color=self.colors[col])
        elif cshape == 'hexagon':
            self.curshape = draw.hexagon(
                self.width // 2 - 75, 300, 75, color=self.colors[col])
        elif cshape == 'triangle':
            self.curshape = draw.triangle(
                self.width // 2 - 75, 300, 150, color=self.colors[col])
        else:
            self.curshape = draw.circle(
                self.width // 2 - 75, 300, 75, color=self.colors[col])
